"""Interactive practice round / sitting orchestrator with hint stepping and FSRS updates."""
...
class InteractiveSitting:
    ...
    def process_item_attempt(
        ...
        card_record = self.fsrs_records.get(
            item.id,
            FSRSRecord(card_id=item.id, due=ref_now),
        )
        updated_card = self.fsrs_engine.schedule_review(card_record, rating=rating, now=ref_now)
    ...
    def get_summary(self) -> RoundSummary:
        """Calculate summary statistics for the completed round."""
...
InteractiveSession = InteractiveSitting
