...
def build_prompt(cefr: CEFR, theme: str, count: int) -> str:
    """The full generation prompt: CEFR level and theme are the only
    grammar-adjacent-looking inputs, and neither names a grammar topic."""
    return f"{_INSTRUCTION}\n\nCEFR level: {cefr}\nTheme: {theme}\nNumber of sentences: {count}\n"


class SentenceGenerator(Protocol):
    def generate(self, cefr: CEFR, theme: str, count: int) -> list[str]: ...
...
class LiveSentenceGenerator:
    ...
    def generate(self, cefr: CEFR, theme: str, count: int) -> list[str]:
        prompt = build_prompt(cefr, theme, count)
...
# randomly generated) spanning the 15 in-scope topics, used whenever no API
# key is configured -- CI, local dev, and every unit test. Reused nowhere
# near verbatim as the pytest test corpus (tests/test_blanking_selectors.py
# hand-checks its own, separately authored sentences) so a selector bug
# cannot hide behind the same fixture serving both roles.
