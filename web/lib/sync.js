...
export async function createGist(token) {
  const res = await fetch(`${API}/gists`, {
    ...
    body: JSON.stringify({ description: "Phrasen review log", public: false, files: { [FILE]: { content: "\n" } } }),
  });
...
export async function pushGist(token, gistId, entries) {
  const res = await fetch(`${API}/gists/${gistId}`, {
    ...
    body: JSON.stringify({ files: { [FILE]: { content: toJsonl(entries) || "\n" } } }),
  });
...
export async function syncLog(settings, local) {
  ...
  if (!gistId) { gistId = await createGist(token); settings.gistId = gistId; }
