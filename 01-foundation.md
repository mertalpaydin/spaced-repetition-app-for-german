...
`eligible_types` is a **correctness constraint, not a stylistic preference**. There is no global item-type quota anywhere in the system; the observed distribution is emergent. Roughly 8 to 12 topics will carry `requires_context: true` (Konjunktiv I, backward-referring connectors, da-compounds, Plusquamperfekt, Nominalstil and register at B2), and those are paragraph-cloze only.
