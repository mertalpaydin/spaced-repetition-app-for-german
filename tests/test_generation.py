...
def test_generation_prompt_asks_for_gloss_en_as_meaning_only(
    all_specs: list[TopicSpec],
) -> None:
    """docs/audits/generation-track-plan.md Cycle 3: the built prompt must
    instruct the model to return a natural English translation of the
    complete sentence (meaning only), never a grammar hint or category
    name, and the required output schema must carry a ``gloss_en`` slot for
    every item."""
    builder = PromptBuilder()
    spec = all_specs[0]
    payload = json.loads(builder.build_generation_prompt(spec, 3, 1))

    assert any("gloss_en" in p and "meaning" in p.lower() for p in payload["prohibitions"]), (
        "prompt must instruct the model to supply gloss_en as meaning-only"
    )
    assert "gloss_en" in payload["required_output_schema"]["items"][0]


def test_generation_prompt_gloss_instruction_itself_leaks_no_grammar_terms(
    all_specs: list[TopicSpec],
) -> None:
    """The instruction telling the model to write ``gloss_en`` bans grammar
    labels like 'Perfekt' by naming them as forbidden examples, which would
    themselves trip ``check_for_topic_leaks`` if scanned as a live prompt.
    That is fine for prose ABOUT the rule; it must never leak into an
    actual gold example or generated sentence. This asserts the reverse
    property actually enforced elsewhere: gold examples for every spec
    stay leak-free even though the instruction text mentioning 'Perfekt' as
    a forbidden example sits in the same prompt payload."""
    builder = PromptBuilder()
    spec = all_specs[0]
    payload = json.loads(builder.build_generation_prompt(spec, 3, 1))

    gloss_instructions = [p for p in payload["prohibitions"] if "gloss_en" in p]
    assert gloss_instructions, "expected a gloss_en instruction in prohibitions"
    for gold in payload["gold_few_shot_examples"]:
        assert builder.check_for_topic_leaks(gold["prompt"]) == []
...
@pytest.mark.parametrize("underscores", ["____", "_____", "______"])
def test_gemini_batch_client_normalizes_underscore_runs_in_parsed_prompt(
    tmp_path: Path, underscores: str
) -> None:
    """A model response whose prompt uses a run of four or more underscores
    (``str.count("___")`` on ``"____"`` is 1 -- the fourth underscore is
    simply left over by non-overlapping matching, so a naive gap-count check
    waves it through) is normalised to exactly three underscores before the
    candidate becomes a ``CandidateItem`` at all."""
        ...
        *, model: str, prompt: str, lane: str, mode: str, purpose: str
        ...
        payload = {
            "items": [
                {
                    "type": "cloze_free",
                    "prompt": f"Das Buch liegt auf {underscores} Tisch.",
                    "proposed_answer": "dem",
                    "distractors": [
                        {"text": "den", "implied_topic_id": "kasus_akkusativ_formen"},
                        {"text": "des", "implied_topic_id": "kasus_genitiv_formen"},
    ...
    assert len(items) == 1
    assert items[0].prompt == "Das Buch liegt auf ___ Tisch."
    assert items[0].prompt.count("___") == 1
    assert "____" not in items[0].prompt


def test_normalize_underscore_runs_collapses_every_run_to_exactly_three() -> None:
    """Pure unit test of the normaliser itself: every run of 3+ underscores
    anywhere in the text becomes exactly ``___``; a run shorter than 3 (not a
    valid gap at all) is left untouched, and text with no underscores is
    unchanged."""
    from src.generation.batch_client import _normalize_underscore_runs

    assert _normalize_underscore_runs("Er wohnt ____ der Ecke.") == "Er wohnt ___ der Ecke."
    assert _normalize_underscore_runs("Er wohnt _____ der Ecke.") == "Er wohnt ___ der Ecke."
    assert _normalize_underscore_runs("Er wohnt ___ der Ecke.") == "Er wohnt ___ der Ecke."
    assert _normalize_underscore_runs("Kein Blank hier.") == "Kein Blank hier."
    assert _normalize_underscore_runs("__ zu kurz, aber ______ zu lang.") == (
        "__ zu kurz, aber ___ zu lang."
    )
    assert _normalize_underscore_runs("a ____ b _____ c") == "a ___ b ___ c"


def test_verify_and_insert_candidates_normalizes_prompt_before_verification(
    tmp_path: Path,
) -> None:
    """The second, unconditional normalisation gate: a hand-fed
    ``CandidateItem`` that never went through ``_parse_response`` (e.g. a
    future generate-then-blank candidate, or a test double) still gets its
    underscore run collapsed to exactly three before the verification chain
    ever sees it."""
    from src.contracts import CandidateItem, Distractor
    from src.generation.batch_client import _verify_and_insert_candidates

    candidate = CandidateItem(
        topic_id="dativ_nach_praeposition",
        type="cloze_free",
        difficulty=1,
        prompt="Das Buch liegt auf ____ Tisch.",
        proposed_answer="dem",
        distractors=[
    ...
    assert result.accepted_items[0].prompt == "Das Buch liegt auf ___ Tisch."
