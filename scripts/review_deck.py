...
CARD_PROMPT = (
    ...
    "'zugrunde richten' bracketed as 'sich richten'). A single-word unit (kind noun, verb, "
    "adjective or adverb) is WRONG_UNIT when the bracketed word is a different lemma or a "
    "different word class than the display, or when the sentence needs a phrase the display "
    "does not name, such as a plain verb bracketed where the sentence uses it reflexively. "
    "WRONG_GAPS (a unit token is missing "
...
UNIT_PROMPT = (
    ...
    "optional +case, cefr=, trivial=, gloss=). Kinds: noun (cited with its article), verb "
    "(an infinitive), adjective, adverb, verb_prep, reflexive_verb, separable_verb, "
    "noun_verb (noun + verb collocation or a Funktionsverbgefuege), adj_verb, adj_noun, "
    "connector, two_part_connector, idiom, expression (a fixed sequence mined from the "
    "corpus, such as 'auf jeden Fall' or 'soweit ich weiss'). The deck teaches single words "
    "as well as phrases, so a one-word unit is expected and is never NOT_A_PHRASE on those "
    "grounds alone. Judge every unit. Report a finding ONLY "
    "when: NOT_A_PHRASE (not worth teaching as a unit: for a multiword unit a free "
    "combination, a parser artefact, a name, news-only jargon or a fragment such as 'kennt "
    "nicht den Unterschied'; for a single word a name, a typo, a numeral, or a form that is "
    "not a citation form); BAD_CITATION (the display "
