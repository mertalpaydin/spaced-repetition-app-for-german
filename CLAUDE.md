...
- Every **attempt** through `src/llm/client.py` writes a row to `cost_log`: timestamp, purpose, lane, **transport mode**, model, input tokens, output tokens, thinking tokens, cached-content tokens, tool-use tokens, the provider's own total token count, the model version Google says it served, estimated cost, and the attempt's **outcome**, **attempt index** and **call id**. An attempt, not a call: a call retried three times before it lands leaves four rows sharing one call id. `outcome`, `attempt` and `call_id` are additive with defaults (`"ok"`, `1`, `None`), so every historical row still parses as exactly what it was, the successful attempt. Flagged per rule 8: this is a change to a persisted format, the second additive one in the same week.
